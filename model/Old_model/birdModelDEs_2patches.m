function birdModelDEs_2patches = birdModelDEs_2patches(t,y)
...
%beta = zeros(n,1); % Conformity parameter
%for i = 1:n
%  beta(i) = 1.5; 
%end
...
for i = 1:n
   if L(i) > 0 | R(i) > 0
    epsilonL(i) = 1 / (1+exp(-log( (L(i)/(L(i)+R(i))) / (1-(L(i)/(L(i)+R(i)))) )*lambda));
   else
    epsilonL(i) = 0;
   end
end
...
for i = 1:n
   if R(i) > 0 | L(i) > 0
    epsilonR(i) =  1 / (1+exp(-log( (R(i)/(L(i)+R(i))) / (1-(R(i)/(L(i)+R(i)))) )*lambda)); % ((R(i)/(L(i)+R(i))) / (1-(R(i)/(L(i)+R(i))))^bet) / (1 + ((R(i)/(L(i)+R(i))) / (1-(R(i)/(L(i)+R(i))))^bet))
   else
    epsilonR(i) = 0;
   end
end
...
birdModelDEs_2patches = transpose(birdModels2);
